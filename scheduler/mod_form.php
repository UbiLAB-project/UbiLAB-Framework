...
class mod_scheduler_mod_form extends moodleform_mod {
    ...
    public function definition() {
        ...
        $activityoptions[get_string('usenotesnone', 'scheduler')] = get_string('usenotesnone', 'scheduler');
        foreach ($availableMods as $activity) {
            $activityoptions[$activity->id] = ucfirst($activity->type) . ": " . ucfirst($activity->name);
        }
        $mform->addElement('select', 'useactivity', 'Select Activity', $activityoptions);

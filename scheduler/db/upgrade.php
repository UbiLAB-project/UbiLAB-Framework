...
function xmldb_scheduler_upgrade($oldversion=0) {
    ...
    if ($oldversion < 2014071300) {
        ...
        // Changing length of field useactivity on table scheduler to (255).
        $table = new xmldb_table('scheduler');
        $field = new xmldb_field('useactivity', XMLDB_TYPE_CHAR, '255', null, XMLDB_NOTNULL, null, null, 'useactivity');

        // Launch change of precision for field useactivity.
        $dbman->change_field_precision($table, $field);
    ...
    if ($oldversion < 2016051700) {
        ...
        $table = new xmldb_table('scheduler');
        $field3 = new xmldb_field('useactivity', XMLDB_TYPE_TEXT, null, null, null, null, null, 'useactivity');
        if (!$dbman->field_exists($table, $field1)) {
            $dbman->add_field($table, $field1);
        }
    ...
    if ($oldversion < 2017040100) {
        ...
        $field = new xmldb_field('useactivity', XMLDB_TYPE_TEXT, null, null, null, null, null, 'useactivity');
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }

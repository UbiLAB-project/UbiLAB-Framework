...
class mod_scheduler_renderer extends plugin_renderer_base {
    ...
    public function render_scheduler_slot_table(scheduler_slot_table $slottable) {
        ...
        $schName = $slottable->scheduler->get_cm()->name;
        $records = $DB->get_records('scheduler',array('name'=>$schName));
        $schOptions = array_pop(array_reverse($records));
        ...
        foreach ($slottable->slots as $slot) {
            ...
            $rowdata[] = $notes;
            // Takes the modul info and extracts its information
            $modinfo = get_fast_modinfo($slottable->scheduler->course);
            $cm = $modinfo->get_cm($schOptions->useactivity);
            $modrec = $DB->get_record($cm->modname, array('id' => $cm->instance));
            // Go zema momentalnoto vreme od serverot i go sporeduva so slot starttime i so slot endtime
            if (time() >= (int)$slot->starttime && time() <= (int)$slot->endtime) {
                //ako e tocno pokazuva link to aktivnosta
                $cmUrl = '/mod/' . $cm->modname . '/view.php';
                $rowdata[] = html_writer::link(new moodle_url($cmUrl, array('id' => $schOptions->useactivity)), $modrec->name);
            } else {
                // ako ne e pokazuva helper tekst
                $rowdata[] = 'Come back at your time </br > slot to see the activity';
            }
    ...
    public function render_scheduler_slot_manager(scheduler_slot_manager $slotman) {
        ...
        $schName = $slotman->scheduler->get_cm()->name;
        $records = $DB->get_records('scheduler',array('name'=>$schName));
        $schOptions = array_pop(array_reverse($records));
        ...
        $table = new html_table();
        $table->head  = array('', get_string('date', 'scheduler'), get_string('start', 'scheduler'),
                        get_string('end', 'scheduler'), get_string('location', 'scheduler'), get_string('students', 'scheduler'), 'Activity' );
        ...
        foreach ($slotman->slots as $slot) {
            ...
            // prema id-to zema informacija az modulot
            $modinfo = get_fast_modinfo($slotman->scheduler->course);
            // od modingfo objektot go stava imeto od modulot
            $rowdata[] = $modinfo->get_cm($schOptions->useactivity)->modname;
